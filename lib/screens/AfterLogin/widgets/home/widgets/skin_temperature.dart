...
class SkinTemperature extends StatefulWidget {
  @override
  _SkinTemperatureState createState() => _SkinTemperatureState();
}

class _SkinTemperatureState extends State<SkinTemperature> with SingleTickerProviderStateMixin {
  // Animation controller for the icon animation
  late AnimationController _animationController;
  late Animation<double> _animation;
  // Stream to fetch skin temperature data
  ...
  void initState() {
    ...
    // Initial update of the skin temperature stream
    _updateHeartRateStream(_currentView);
  }

  // Function to update the skin temperature stream based on the selected view
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            // Skin temperature display section
            Row(
            ...
            StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                try {
                  final skinTemperatureData = snapshot.data!.docs.map((doc) {
                    return doc['reading'].toDouble();
                  }).toList();
                  final double avgSkinTemperature = skinTemperatureData.reduce((a, b) => a + b) / skinTemperatureData.length;
                  final double minSkinTemperature = skinTemperatureData.reduce((a, b) => a < b ? a : b);
                  final double maxSkinTemperature = skinTemperatureData.reduce((a, b) => a > b ? a : b);
                  return Column(
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          _buildMetricCard('AVG', avgSkinTemperature),
                          _buildMetricCard('MIN', minSkinTemperature),
                          _buildMetricCard('MAX', maxSkinTemperature),
                        ],
                      ),
                      const SizedBox(height: 20),
                    ],
            ...
            // Line chart to display skin temperature data
            StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
              stream: _heartRateStream,
              builder: (context, snapshot) {
                if (snapshot.hasError) {
                ...
                try {
                  ...
                  return Container(
                    height: 200,
                    width: 300,
                    child: _buildLineChart(skinTemperatureData.reversed.toList()),
                  );
                } catch (e) {
                  return Container(
                    height: 200,
